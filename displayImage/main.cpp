...
namespace GameLib{
	void Framework::update(){
		if ( gFirst ){
			...
			//////////////////
			cout << "dwMagic: " << getUnsigned(&(buffer[0])) << endl;
			cout << "dwSize: " << getUnsigned(&(buffer[4])) << endl;
			cout << "dwFlags: " << getUnsigned(&(buffer[8])) << endl;
			/////////////////
...
#if 0
	for (int y = 0; y < gImageHeight - 32; ++y) {
			for (int x = 0; x < gImageWidth - 32; ++x) {
